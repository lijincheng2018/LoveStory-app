<template>
	<view class="bg">
		...
		<view class="Heading-title">添加心碎事件</view>
		...
		<view class="subTitle">{{ eventInfo.time }}
			<view class="changeButton" style="top: 5px;" @click="changeDate()">修改</view>
		</view>
		...
		<view class="shopBox">
			...
			<view class="labelContent">
				...
				<view class="changeButton" style="top: 60px;" @click="isChangeTitle === false ? changeTitle() : saveTitle()">{{ isChangeTitle === false ? '修改' : '保存' }}</view>
			</view>
			<view class="labelTitle">减少心跳值：</view>
			<view class="labelContent">
				...
				<view class="changeButton" style="top: 140px;" @click="isChangeValue === false ? changeValue() : saveValue()">{{ isChangeValue === false ? '修改' : '保存' }}</view>
			</view>
			<view class="labelTitle">描述：<view class="changeButton" style="top: 195px;" @click="isChangeContent === false ? changeContent() : saveContent()">{{ isChangeContent === false ? '修改' : '保存' }}</view>
			</view>
			<view class="Wordcanves">
				<view class="content">
					<span v-if="isChangeContent === false">{{ eventInfo.content }}</span>
					<textarea v-else-if="isChangeContent === true" class="labelContentTextarea" v-model="new_content"></textarea>
				</view>
				
			</view>
			<view class="submitButton" @click="SubmitEvent">确认提交</view>
		</view>
...
<script>
	export default {
		...
		onShow() {
			...
			const time = `${year}年${month}月${day}日 ${timePeriod}`; // 拼接日期时间格式
			this.eventInfo.time = time
		},
		methods: {
			// 返回心跳值页
			toheartValue() {
			...
			changeContent() {
				...
				this.new_content = this.eventInfo.content
			},
			// 保存描述
			saveContent() {
			...
			SubmitEvent() {
				...
				//发送请求
				uni.request({
					url: "https://lovestory.ljcljc.cn/api/addEvent",
					method:'POST',
					header: {
